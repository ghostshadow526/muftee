...
export const useFirebaseComplaints = () => {
  ...
  const addComplaint = async (complaintData: {
    ...
    if (!currentUser) {
      throw new Error("User must be logged in to submit complaints");
    }
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, "complain"), newComplaint);
      return docRef.id;
    } catch (error) {
  ...
  const updateComplaintStatus = async (
    ...
    try {
      const complaintRef = doc(db, "complain", complaintId);
  ...
  const deleteComplaint = async (complaintId: string) => {
    ...
    try {
      await deleteDoc(doc(db, "complain", complaintId));
    } catch (error) {
